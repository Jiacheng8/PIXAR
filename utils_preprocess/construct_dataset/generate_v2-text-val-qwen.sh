...
BASE_OUT_DIR="/data/ironman/jiacheng/final_Omni_Data"
...
LOG_DIR="./logs"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/construct_unified_text_${TYPE}_${RUN_ID}.log"
...
run_one () {
  ...
  log "▶️  Start: id=${id} | tao=${tao} | anno=${anno_flag} | bg=${bg_flag} | dest=${dest_type} | out=${out_dir}"
...
# Main
# -------------------------
h1 "Construct TYPE=${TYPE} (run_id=${RUN_ID})"
log "📂 workdir=$(pwd)"
