...
def main():
    ap = argparse.ArgumentParser(description="Construct dataset from provided parameters (nested ann_id + save real once per name).")
    ap.add_argument("--id", required=True, help="Path to the dataset file (dataset id under dataset-dir).")
    ...
    ap.add_argument("--dest-type", default="train", type=str, choices=["train", "test", "validation"])
    ...
    # 只保存一次 real 的标记：key 用 name（第一层目录名）
    saved_real_flag = {}
    ...
    # 构建 tasks：dataset_path/<name>/<ann_id>/
    tasks = []
    ...
    def process_one(task) -> int:
        ...
        # tampered ID：把 ann_id 也纳入（避免同 name 下冲突）
        tampered_id = hash_string(f"{args.id}/{name}/{ann_id}", length=16)

        # real ID：仍然基于 original.png 内容（稳定）
        try:
        ...
        # 目标路径（tampered/mask/soft/meta 按 dest_type；real 统一 train/real）
        ...
        with mapping_lock:
            reverse_mapping[tampered_filename] = {
                "entry": entry,          # name/ann_id
                "real": real_filename,   # 指向同一个 real 文件名
                "type": args.id
            }

        # ✅ 只对同一个 name 保存一次 real（但文件名仍是内容 hash，跨 run 稳定）
        try:
